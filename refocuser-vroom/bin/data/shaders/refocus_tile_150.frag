...
void main (void){
    ...
    for (int x=ap_loc.x; x <(ap_loc.x+ap_size.x); x++){
        for (int y=ap_loc.y; y<(ap_loc.y+ap_size.y); y++) {
            ...
            vec2 offset = texture(campos_tex, cam_num).xy - texture(campos_tex, ap_center).xy;
            vec2 shift = fscale * offset;

            vec2 pos = subimg_corner + pixelpos - shift;

            int tilenum = int(texture(tilenum_tex, cam_num).x);
            vec2 tilepixoffset = texture(tilepixoffset_tex, cam_num).xy;

            color += texture(lftex[tilenum], pos-tilepixoffset);
